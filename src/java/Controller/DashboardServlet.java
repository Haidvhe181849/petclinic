...
public class DashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Employee ep = (Employee) session.getAttribute("staff");

        // Cho phép nếu: user tồn tại và role 1 hoặc 2, hoặc staff tồn tại
        if ((user != null && (user.getRoleId() == 1 || user.getRoleId() == 2)) || ep != null) {
            try (Connection conn = new DBContext().connection) {

                // Xử lý tham số filter (mặc định là tuần)
                ...
                // Doanh thu cố định để hiển thị thống kê
                request.setAttribute("weeklyRevenue", getWeeklyRevenue(conn));
                request.setAttribute("monthlyRevenue", getMonthlyRevenue(conn));
                request.setAttribute("yearlyRevenue", getYearlyRevenue(conn));
                ...
                e.printStackTrace();
                request.setAttribute("error", "Không thể tải dữ liệu bảng điều khiển.");
